...
from  TGCN.artificially_feature import art_features
from .TGCN.handler import _preprocess_superpixels,_cross_entropy
from .TGCN.handler import _adj,art_adj,smoothness_reg,load_model_weights
...
class FSTGNTrainer(BaseTrainer):
    ...
    def get_default_optimizer(self):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, 'min', patience=6, factor=0.5, min_lr=1e-5, verbose=True)
    ...
    def compute_loss(self, pred, target, metrics=None):
        ...
        if self.kwargs.get('is_gcn'):
            ...
            gcn_loss = 0.25 * gcn_loss / gcn_out.shape[0]
            gcn_loss = gcn_loss.clamp(0.001, 0.3)  # 限制范围
            loss += gcn_loss
            ...
            art_regloss = 0.25 * art_gcn_loss / art_gcn_out.shape[0]
            art_regloss = art_regloss.clamp(0.001, 0.2)  # 限制手工特征正则化范围
            loss += art_regloss
...
class FSTGNPixelInference(FSTGN):
    """Weakly supervised histopathology image segmentation with sparse point annotations.
        稀疏点标注的组织病理图像分割。 """
