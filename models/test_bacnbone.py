...
class Test_BackboneConfig(BaseConfig):
...
class Test_Backbone(nn.Module):
    ...
    def __init__(self, n_classes=2, D=32, **kwargs):
        ...
        # unet 特征提取==================
        for name, layer in self.backbone.decoder.named_modules():
    ...
    def forward(self, x):
        ...
        """
        # x, sp_maps = x
        self.fm_size = (512,512)
        sp_maps = torch.rand((101,512,512)).cuda()
...
class Test_BackboneTrainer(BaseTrainer):
    """Trainer for TGCN."""
...
class TGCNPixelInference(Test_Backbone):
    """Weakly supervised histopathology image segmentation with sparse point annotations."""
